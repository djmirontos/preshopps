/** Real PostgreSQL integration test for migration 0100 (Fair-condition
 * Draft fix), independently runnable from published-listing-editing.mjs.
 * PRESHOPPS_TEST_PSQL=/path/to/psql PRESHOPPS_TEST_PORT=55494 node tests/database/known-flaws-draft-fix.mjs
 ...
 * Replays every repository migration 0001-0100 in order (0086 permanently
...
const database = `preshopps_test_0100_known_flaws_${process.pid}_${Date.now()}`;
...
try {
  ...
  if(migrations.at(-1)!=='0100_allow_incomplete_fair_condition_draft.sql')
    throw new Error('0100_allow_incomplete_fair_condition_draft.sql must be the latest migration file.');
  if(migrations.some(f=>f.startsWith('0086_')))
    throw new Error('0086 must remain permanently absent.');
  if(migrations.filter(f=>f.startsWith('0100_')).length!==1)
    throw new Error('Exactly one 0100 migration file must exist.');
  ...
  console.log(`REPLAY: ${migrations.length} repository migrations through 0100 ran in order; 0051 HTTP and 0084 cron/net used documented test shims.`);

  sql(file('tests/database/known-flaws-draft-fix-fixtures.sql'));
  check('all ten known-flaws-draft-fix cases (0100 invariant, publish enforcement, CHECK constraint, unrelated regressions)',
    file('tests/database/known-flaws-draft-fix.sql'));
