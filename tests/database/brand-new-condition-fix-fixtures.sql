-- Test identity/category/location fixtures for brand-new-condition-fix.mjs
-- only. Deliberately minimal -- this suite is independently runnable and
-- does not share fixtures or state with published-listing-editing.mjs or
-- known-flaws-draft-fix.mjs. Two sellers/shops exist (unlike the
-- known-flaws-draft-fix fixtures' one) because this suite must prove an
-- unauthorized cross-seller publish attempt cannot repair another
-- seller's row.
...
('10000000-0000-0000-0000-000000000001','seller-a@example.invalid',now(),'{"display_name":"Seller A"}'),
('10000000-0000-0000-0000-000000000002','seller-b@example.invalid',now(),'{"display_name":"Seller B"}');
...
insert into public.shops(id,owner_id,name,slug,province_id,city_id) values
('20000000-0000-0000-0000-000000000001','10000000-0000-0000-0000-000000000001','Seller A shop','seller-a-shop',900001,900001),
('20000000-0000-0000-0000-000000000002','10000000-0000-0000-0000-000000000002','Seller B shop','seller-b-shop',900001,900001);
