...
describe("Buy Now -- migration 0089 exists and touches nothing else", () => {
  it("0089_buy_now_order_submission.sql exists, and no migration newer than 0090 exists yet (0090 is a later, separately-approved update_listing ambiguity fix, unrelated to Buy Now)", () => {
...
describe("Buy Now -- post-submit terminal-success fix (P2) stayed frontend-only", () => {
  it("no migration newer than 0090 exists -- this fix touched no backend/RPC/schema (0090 is a later, separately-approved update_listing ambiguity fix)", () => {
    const migrationFiles = readdirSync(path.join(process.cwd(), "supabase/migrations")).filter((f) => f.endsWith(".sql"));
    const newerThan0090 = migrationFiles.filter((f) => f > "0090_fix_update_listing_listing_id_ambiguity.sql");
    expect(newerThan0090).toEqual([]);
  });
...
describe("Buy Now -- post-submit navigation fix (P3): straight to order detail, no intermediate success card", () => {
  it("no migration newer than 0090 exists -- this fix is frontend-only (0090 is a later, separately-approved update_listing ambiguity fix)", () => {
    const migrationFiles = readdirSync(path.join(process.cwd(), "supabase/migrations")).filter((f) => f.endsWith(".sql"));
    const newerThan0090 = migrationFiles.filter((f) => f > "0090_fix_update_listing_listing_id_ambiguity.sql");
    expect(newerThan0090).toEqual([]);
  });
