...
import { markNotificationRead, markAllNotificationsRead, dismissNotification, dismissAllNotifications } from "@/lib/notifications/notification-actions";
...
describe("dismissNotification", () => {
  it("calls dismiss_notification with only the notification id (no client-supplied user id)", async () => {
    rpcMock.mockResolvedValue({ data: null, error: null });
    await dismissNotification("notif-1");
    expect(rpcMock).toHaveBeenCalledWith("dismiss_notification", { p_notification_id: "notif-1" });
    const args = rpcMock.mock.calls[0][1];
    expect(Object.keys(args)).toEqual(["p_notification_id"]);
  });
...
describe("dismissAllNotifications", () => {
  it("calls dismiss_all_notifications with no arguments", async () => {
    ...
    expect(rpcMock).toHaveBeenCalledWith("dismiss_all_notifications");
  });

  it("returns ok: true with the dismissed count on success", async () => {
    rpcMock.mockResolvedValue({ data: 3, error: null });
    const result = await dismissAllNotifications();
    expect(result).toEqual({ ok: true, dismissedCount: 3 });
  });
