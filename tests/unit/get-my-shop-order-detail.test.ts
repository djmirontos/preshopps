...
describe("getMyShopOrderDetail", () => {
  it("calls get_my_shop_order_detail with only the public code (no client-supplied shop/seller id)", async () => {
    rpcMock.mockResolvedValue({ data: [row()], error: null });
    await getMyShopOrderDetail("PSO-ABC12345");
    expect(rpcMock).toHaveBeenCalledWith("get_my_shop_order_detail", { p_public_code: "PSO-ABC12345" });
  });

  it("returns not_found when the RPC returns zero rows (another seller's order or a nonexistent code -- indistinguishable)", async () => {
    rpcMock.mockResolvedValue({ data: [], error: null });
    const result = await getMyShopOrderDetail("PSO-GONE");
    expect(result).toEqual({ status: "not_found" });
  });

  it("maps a single-item order to SellerOrderDetail, deriving totalCents from the items", async () => {
    rpcMock.mockResolvedValue({ data: [row()], error: null });
    const result = await getMyShopOrderDetail("PSO-ABC12345");
  ...
  it("never exposes a buyer email in the mapped result", async () => {
    rpcMock.mockResolvedValue({ data: [row()], error: null });
    const result = await getMyShopOrderDetail("PSO-ABC12345");
