-- Seller Listing Management, canonical correction: resolves a genuine
-- implementation defect against unanimous canonical text (PRD 9.3/10.1/10.6,
-- ARCHITECTURE.md "Draft vs publish validation boundary", ARCHITECTURE_
-- ESSENTIALS.md, AGENTS.md, CLAUDE.md). All five sources agree with each
-- other: title is the only field required to create/save a Draft, condition
-- is explicitly named among the fields a Draft need not have, and "Fair
-- requires known-flaws text" is listed everywhere as a PUBLISH-time
-- requirement (PRD 10.1's own "fields above are what a listing must have to
-- be published"), never a Draft-time one. There is no canonical contradiction
-- to resolve here -- only an RPC/constraint implementation that never
-- correctly applied that already-unanimous rule to the Known-Flaws case,
-- unlike every other publish-ready field (description, category, condition,
-- price, images, etc.), which 0055/0056 already correctly deferred to
-- publish time.
--
-- Owner-approved design (LAUNCH UX S1.1, read-only audits performed before
-- this file was written -- both independently confirmed the same root cause
-- and the same minimal fix):
--   Draft + Fair + blank Known Flaws: allowed.
--   Non-Draft + Fair + blank Known Flaws: rejected.
--   Publish still requires Known Flaws for Fair (unchanged enforcement point).
--
-- Root cause (two duplicated, status-blind guards; one already-correct,
-- already-shared guard)
-- -----------------------------------------------------------------------
-- Three places check "Fair requires Known Flaws" today. Only two are wrong:
--   1. create_listing (latest: 0082_harden_anonymized_account_mutations.sql)
--      -- raises KNOWN_FLAWS_REQUIRED unconditionally, even though this
--      function always inserts status='draft' literally. Dead-wrong code:
--      the condition it guards against should never apply to a Draft at all.
--   2. update_listing (latest: 0090_fix_update_listing_listing_id_ambiguity.sql)
--      -- identical unconditional guard, even though this function
--      unconditionally rejects (LISTING_NOT_DRAFT) any row that is not
--      already status='draft' before reaching this check. Same dead-wrong
--      code for the same reason.
--   3. validate_published_listing (0094_published_listing_editing.sql) --
--      already correct and already the single shared enforcement point for
--      every path that can move a listing to/through 'available'
--      (publish_listing, update_listing_status's paused->available
--      transition, and update_published_listing's re-validation on every
--      save). This function is NOT touched by this migration.
--
-- The `listings_fair_requires_known_flaws_check` CHECK constraint
-- (0008_listings.sql) is the actual blocker independent of the two RPC
-- guards above: it is status-blind today, so even after removing the RPC
-- guards, the table itself would still reject a Draft row with
-- condition='fair' and blank known_flaws. This migration makes the
-- constraint status-aware; validate_published_listing's own inline check
-- (unchanged) remains the reason a non-draft row can never carry that state
-- in the first place, so the two enforcement layers stay in agreement.
--
-- Why 0082/0090/0008 are not edited in place
-- -----------------------------------------------------------------------
-- All three are already applied live. Per this project's repeatedly-
-- established rule (0050, 0055, 0056, 0090's own headers for the identical
-- situation), an already-applied migration file is never edited -- a
-- correction gets its own new file. 0008/0082/0090 are left byte-for-byte
-- untouched; this migration is create_listing's and update_listing's next,
-- current home, and the constraint's next, current home. Both functions'
-- parameter lists (names, types, order, count) are completely unchanged, so
-- both are CREATE OR REPLACE FUNCTION against identical signatures, not a
-- DROP + re-create.
--
-- Existing-data safety (verified by construction, no runtime data audit
-- required)
-- -----------------------------------------------------------------------
-- The new constraint predicate (`status = 'draft' OR <old predicate>`) is a
-- strict superset of what the current constraint already accepts -- it only
-- ever accepts MORE rows, never fewer. Every row already live today already
-- satisfies the stricter old constraint (Postgres would not have allowed it
-- otherwise), so every existing row automatically satisfies the new one too,
-- regardless of current row count or data shape. Added NOT VALID + validated
-- immediately after as the safer default lock profile: NOT VALID takes only
-- a brief ACCESS EXCLUSIVE lock for the metadata change (no scan), and the
-- following VALIDATE CONSTRAINT takes the much lighter SHARE UPDATE
-- EXCLUSIVE (blocks other DDL only, not ordinary reads/writes) while it
-- scans -- a strictly safer choice even though the scan is mathematically
-- guaranteed to pass.
--
-- Function changes: precisely scoped, nothing else touched
-- -----------------------------------------------------------------------
-- create_listing: the entire function body is copied unchanged from 0082
-- except that the 2-line `if p_condition = 'fair' and v_known_flaws is null
-- then raise ... 'KNOWN_FLAWS_REQUIRED'; end if;` block is deleted. The
-- `v_known_flaws := nullif(btrim(p_known_flaws), '');` assignment
-- immediately above it is kept unchanged (still used by the later INSERT).
-- update_listing: identical treatment -- only the `if v_final_condition =
-- 'fair' and v_final_known_flaws is null then raise ... end if;` block from
-- 0090 is deleted; `v_final_known_flaws`'s own assignment logic is kept
-- unchanged (still used by the later UPDATE). No other line in either
-- function -- authentication, deleted-account eligibility, shop ownership,
-- seller-suspension/account-suspension restriction checks, title
-- requirement, every other field's validation, slug/public_code generation,
-- atomicity, side effects, or return shape -- is touched. Grants are
-- reissued identically to their current live shape (REVOKE ALL FROM public/
-- anon, GRANT EXECUTE TO authenticated only), matching this project's own
-- established convention of always reissuing them explicitly on a
-- same-signature CREATE OR REPLACE, even though Postgres already preserves
-- them automatically in that case.
--
-- Objects deliberately NOT touched by this migration: publish_listing,
-- update_listing_status, update_published_listing, validate_published_
-- listing, replace_listing_images, get_published_listing_edit_state, every
-- RLS policy, every Storage policy, every listing enum/column. publish_listing
-- already delegates entirely to validate_published_listing (unchanged),
-- which already enforces this rule correctly at the Draft -> Available
-- transition; update_published_listing can never reach a Draft row
-- (LISTING_NOT_EDITABLE gate) and already re-validates via the same shared
-- validator on every save; update_listing_status already calls the same
-- shared validator only on its paused -> available transition.
--
-- Security: unchanged from 0082/0090 for both functions -- SECURITY
-- DEFINER, set search_path = '', identical REVOKE/GRANT shape, no
-- client-supplied owner/public_code/slug/status/cover_image_id, one
-- implicit transaction per call. No RLS, Storage policy, or grant on any
-- other object is touched. No new schema object, enum, or index is added.

alter table public.listings
  drop constraint listings_fair_requires_known_flaws_check,
  add constraint listings_fair_requires_known_flaws_check
    check (
      status = 'draft'
      or condition <> 'fair'
...
alter table public.listings
  validate constraint listings_fair_requires_known_flaws_check;
